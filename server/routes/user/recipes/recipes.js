...
async function createNewRecipe(recipeData, user) {
  ...
  await newRecipe.setIngredientsLists(
    await createIngredientsLists(recipeData, newRecipe)
  )
...
async function createIngredientsLists(recipeData, newRecipe) {
  ...
  if (isFromType(ingredients, Object)) {
    for (var title in ingredients) {
      let ingredientsList = await createIngredientsList(
        ingredients[title],
        title,
        newRecipe
      )
...
async function recipesToJson(recipes) {
  ...
  await asyncForEach(recipes, async (recipe, i, arr) => {
    ...
    await asyncForEach(recipe.IngredientsLists, async (list, i, arr) => {
      ...
      ingredientsLists.push({
        title: list.dataValues.title,
        ingredients: rawIngredients.map(ing => ing.toJson())
      })
    })
    ...
    await asyncForEach(recipe.StepsLists, async (list, i, arr) => {
      ...
      stepsLists.push({
        title: list.dataValues.title,
        steps: rawSteps.map(s => s.toJson())
      })
    })

    formatedRecipe.ingredientsLists = ingredientsLists
    formatedRecipe.stepsLists = stepsLists
